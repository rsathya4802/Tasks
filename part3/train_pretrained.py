...
writer = SummaryWriter('runs/mnist3_pretrained')
...
load_chkpoint(torch.load('./pretrained_custom_mnist.pth.tar',map_location=device))
# Loss and optimizer
...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_acc:
            ...
            save_chkpt(chkpt,'part3_pretrained_model.pth.tar')

def test_model():

    model = create_model(1, [1, 2, 2, 2], 10).to(device)

    load_chkpoint(torch.load('./part3_pretrained_model.pth.tar'),model)
